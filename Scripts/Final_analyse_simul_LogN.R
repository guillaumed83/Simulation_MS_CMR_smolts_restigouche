...
load("C:/Users/DauphinGu/Desktop/smolts_restigouche/Model_CMR/2020-07-07_clean_scripts_data_2019/Output/_simulation_coda_R2jags_final_update_2022-12-14_M1-M4_LogN.RData")
...
legend("topleft",legend=c("indpt_merge_no_hier",
                          "indpt_no_hier",
                          "indpt_hier","indpt_hier_split","DM"),
...
for(i in 1:10){
  ...
  plot(temp_split_sum$`50%`[y_wheels[,4]==1],temp_theta4_sum$`50%`,pch=16)
  plot(temp_split_sum$`50%`[y_wheels[,5]==1],temp_theta5_sum$`50%`,pch=16)
  
}



png("Figures/2022-12-15/LogN/Simulation_total_abundance.png",height=1300,width=1800,pointsize = 32)
...
png("Figures/2022-12-15/LogN/Simulation_CV_total_abundance.png",height=1800,width=1300,pointsize = 32)
...
png("Figures/2022-12-15/LogN/Simulation_diff_total_abundance.png",height=1200,width=1800,pointsize = 32)
...
tiff("Figures/2022-12-15/LogN/diff_Nm_tot_ggplot.tiff", units="in", width=20, height=15, res=250)
...
for( i in 1:length(all_simul)){
  ...
  gg1 <- gg1 + 
    ...
    geom_vline(xintercept = 0,col="grey65",linetype="dashed",size=1.5)+
    geom_vline(xintercept = mean(dplyr::bind_rows(temp_list)$value),col="red",linetype="dashed",size=1.5  ) + 
    annotate("text",x=-Inf,y=14.5, label = paste("Avg. Diff. = ",round(mean(dplyr::bind_rows(temp_list)$value),2)),  hjust = -0.1) +
    xlab("diff. Total abundance") +
    #xlim(0,2.5)+
    theme(legend.position="none")
...
tiff("Figures/2022-12-15/LogN/diff_Nm_tot_pcent_ggplot.tiff", units="in", width=20, height=15, res=250)
...
write.csv(round(summary_ratio_diff_true,2),"outputs/2022-12-15/LogN/ratio_diff_true.csv")
...
write.csv(round(summary_ratio_diff_true_N1,2),"outputs/2022-12-15/LogN/ratio_diff_true_N1.csv")
...
tiff("Figures/2022-12-15/LogN/diff_N1_ggplot.tiff", units="in", width=20, height=15, res=250)
...
for( n in 1:length(all_simul)){
  ...
  tiff(paste("Figures/2022-12-15/LogN/proportions_",simul_names[n],".tiff",sep=""),
...
tiff("Figures/2022-12-15/LogN/Nm_tot_ggplot.tiff", units="in", width=20, height=15, res=250)
...
tiff("Figures/2022-12-15/LogN/N1_ggplot.tiff", units="in", width=20, height=15, res=250)
...
for(j in 1:10){
  ...
  plot(density(jags_DM_simul[[j]]$BUGSoutput$sims.list$eta_alphaN))
  abline(v=15,col="red")
  sum_eta[j,]<-sum_fun(jags_DM_simul[[j]]$BUGSoutput$sims.list$eta_alphaN)
}
